...
class Tutor
	...
	def validarDatos
        ...
        raise  "El numero de dni es invalido : #{dni_tutor}" if (dni_tutor.to_s.length != 8)
		raise  "El parentesco no se ha ingresado: #{parenteco}" if (parentesco==nil)
	end
end
...
class Alumno
	...
	def validarDatos
		...
		raise  "La edad no se ha ingresado. de 11 a 15" if (edad_alumno==nil)
		raise  "El genero no se ha ingresado. Masculino o Femenino" if (genero_alumno==nil || genero_alumno=="")
		raise  "El numero de dni es invalido : #{dni_alumno}" if (dni_alumno.to_s.length != 8 )
...
class Administrador
	...
	def simuladorExamen(codigoExamen)
		...
		for alumno in lista_postulantes
			...
			loop do
				cont = 0
				for nPreg in examen.listaPreguntas
                    correcto, opcionResp = rand(0..1) , nil
                    if correcto == 1
                        cont = cont + 1
                        for a in examenAlu.listaRespuestasValida
                            if a.codigo == nPreg.codigo then opcionResp = a.opcion end
                        end
                        examenAlu.registrarRespuestaAlumno( nPreg.codigo, opcionResp )
                    else
                        examenAlu.registrarRespuestaAlumno( nPreg.codigo, opcionAlt[rand(0..5)] )
                    end
                end 
